...
def patch_test_file(filepath):
    ...
    content = content.replace(
        "-device zenoh-clock,mode=suspend,node=0", "-device zenoh-clock,mode=suspend,node=0,router=tcp/127.0.0.1:7447"
    )
    content = content.replace(
        "-device zenoh-clock,mode=icount,node=0", "-device zenoh-clock,mode=icount,node=0,router=tcp/127.0.0.1:7447"
    )
    content = content.replace(
        "-device zenoh-clock,mode=icount,node=1", "-device zenoh-clock,mode=icount,node=1,router=tcp/127.0.0.1:7447"
    )
