...
@pytest.mark.asyncio
async def test_clock_slaved_suspend_smoke(simulation: Simulation, guest_app_factory: Any) -> None:  # noqa: ANN401
    """
    ...
    async with simulation as sim:
        ...
        vtime = (await sim.vta.step(0))[0]  # LINT_EXCEPTION: vta_step_loop
        ...
        # 2. Advance 10ms
        vtime = (await sim.vta.step(10_000_000))[0]  # LINT_EXCEPTION: vta_step_loop
...
@pytest.mark.asyncio
async def test_clock_stall_detection(simulation: Simulation, guest_app_factory: Any) -> None:  # noqa: ANN401
    """
    ...
    async with simulation as sim:
        ...
        try:
            with pytest.raises(RuntimeError, match="reported CLOCK STALL"):
                ...
                await sim.vta.step(10_000_000, timeout=(base_stall / 1000.0) + 10.0)  # LINT_EXCEPTION: vta_step_loop

            assert sim.bridge is not None
            await sim.bridge.start_emulation()
            # Give QEMU a moment to resume
            vtime = (await sim.vta.step(1_000_000))[0]  # LINT_EXCEPTION: vta_step_loop
            assert vtime > 0
...
@pytest.mark.asyncio
async def test_slow_boot_fast_execute(simulation: Simulation, guest_app_factory: Any) -> None:  # noqa: ANN401
    """
    ...
    async with simulation as sim:
        ...
        await sim.bridge.pause_emulation()
        with pytest.raises(RuntimeError, match="reported CLOCK STALL"):
            await sim.vta.step(1_000_000, timeout=(base_stall / 1000.0) + 5.0)  # LINT_EXCEPTION: vta_step_loop


@pytest.mark.asyncio
async def test_clock_suspend_wfi(simulation: Simulation, guest_app_factory: Any) -> None:  # noqa: ANN401
    """
    ...
    async with simulation as sim:
        ...
        vtime = (await sim.vta.step(20_000_000))[0]  # LINT_EXCEPTION: vta_step_loop
        assert vtime >= 20_000_000
...
@pytest.mark.asyncio
async def test_clock_suspend_vtime_alignment(simulation: Simulation, guest_app_factory: Any) -> None:  # noqa: ANN401
    """
